...
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  ...
  console.log("🔧 [generateStaticParams] Found slugs:", posts.map(p => p.slug));
...
export async function generateMetadata(props: { params: { slug: string } }): Promise<Metadata> {
  const { params } = await Promise.resolve(props); // Ensures async resolution
  ...
  if (!post) return {};
  console.log("🔍 [generateMetadata] Looking for:", slugPath);
console.log("📚 Available:", posts.map(p => p.slug));
...
export default async function Project({ params }: { params: { slug: string } }) {
  console.log("📦 Page rendered with slug:", params.slug);
  return <Text>✅ Page rendered successfully</Text>;
}
...
//   //       ) : (
//   //         <Text>Invalid or missing content</Text>
//   //       )}
//   //       </div>
//   //     </Column>
//   //     <ScrollToHash />
